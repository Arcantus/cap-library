...
public interface TrustStrategy {
  ...
  /**
   * Return { false} if valid digital signatures are always required.
   * Return { true} if a missing digital signature or Public Key should
  ...
  /**
   * Return { false} if we must always have explicit trust in the
   * credentials used to validate a digital signature (as defined by
   * {@link #isKeyTrusted}).
   * Return { true} if untrusted credentials should be tolerated.
   */
  boolean allowUntrustedCredentials();

  /**
   * Return { true} if the given PublicKey should be trusted in the
   ...
   * Return { false} if the given PublicKey is not trusted.
   *
